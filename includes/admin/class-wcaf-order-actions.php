...
if ( ! class_exists( 'WCAF_Order_Actions' ) ) {

	/**
	 * Class WCAF_Order_Actions
	 */
	class WCAF_Order_Actions {
		 ...
		 *
		 * @var ?WCAF_Order_Actions $instance
		 */
		protected static $instance = null;

		/**
		 * WCAF_Order_Actions constructor.
		 */
		...
		public static function add_new_order_action( $order_actions ) {
			...
			if ( ! empty( $action ) && ! empty( $order_id ) ) {
				...
				$order_actions['black_list_order'] = __( 'Blacklist order', 'wc-anti-fraud' );
				// Check if the order details of this current order is in blacklist.
				$customer = wcaf_get_customer_details_of_order( $order );
				if ( false !== $customer && WCAF_Blacklist_Handler::is_blacklisted( $customer ) ) {
					$order_actions['remove_from_black_list'] = __( 'Remove from Blacklist', 'wc-anti-fraud' );
				}
			}
		...
		public static function update_blacklist( $post_id, $post ) {
			...
			if ( ! empty( $action ) ) {
				...
				if ( 'black_list_order' === $action ) {
					...
					if ( method_exists( 'WCAF_Blacklist_Handler', 'init' ) ) {
						WCAF_Blacklist_Handler::init( $customer, $order, 'add', 'back' );
					}
				} elseif ( 'remove_from_black_list' === $action ) {
					// Remove the customer details from blacklist.
					// update the blacklists.
					if ( method_exists( 'WCAF_Blacklist_Handler', 'init' ) ) {
						WCAF_Blacklist_Handler::init( $customer, $order, 'remove', 'back' );
					}
				}
			}
		}
